package com.ziggfreed.kweebec.camera;
...
/**
 * Server-driven camera override (the survival-mode top-down flex; part of the
 * shared core). Sends the {@code SetServerCamera} packet (280) via the player's
 * packet handler - the exact transport {@code CameraShakeService} uses. The
 * top-down construction mirrors the native {@code PlayerCameraTopdownCommand}
 * field-for-field; reset mirrors {@code PlayerCameraResetCommand}.
 *
    ...
    public static void apply(@Nonnull PlayerRef playerRef, @Nonnull ClientCameraView view,
        ...
        try {
            ...
            KweebecNightmarePlugin.LOGGER.atFine().log(
                    "[Kweebec] camera apply failed: " + t.getMessage());
        }
    }

    /** Survival top-down: locked overhead camera (exact engine recipe). */
