...
/**
 * Lang-backed chrome for the PvP (Clash + Domination) results screen. Mirrors
 * {@link KweebecResultsMessages} but reuses Kweebec's RESULTS_* outcome / duration / reward keys
 * (mode-agnostic) and names the per-player breakdown for a team brawl ("Combat" points, "Round
 * Stats" raw counts). The team-total label is shown above each side in the two-team layout.
 */
public final class KweebecClashResultsMessages implements ResultsMessages {
    ...
    @Override @Nonnull public Message breakdownTitle() {
        return Lang.msg(Lang.RESULTS_CLASH_BREAKDOWN);
    }
