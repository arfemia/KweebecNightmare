...
- **The leaderboard is the generalized `ziggfreed-common` primitive** (`instance/leaderboard/`), wired in
  ...
  total points + the per-stat counters (`stunned`/`moonbloom`/`shrines`/`wins`). The page shows a difficulty x
...
- **Wiring:** `RoundService.resolve` fires `RoundCompletedEvent` + `InstanceCompletedEvent`, then hands off
  to the mode's `onResolve`; `ChaseRoundMode.onResolve` computes the per-player scores, fires
  `RoundEvents.fireRoundScored`
